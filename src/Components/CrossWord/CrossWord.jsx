...
const generateSimpleCrossword = (wordPairs) => {
	...
	for (let i = 1; i < sortedPairs.length; i++) {
		...
		if (!placed) {
			...
			if (!placed) console.warn(`Could not place: ${word}`);
		} else {
			usedWords.push(word);
		}
	}
					...
					<table className={`board`} style={{ borderCollapse: 'collapse' }}>
						<tbody>
							{grid.map((row, rIdx) => (
								<tr key={rIdx}>
									{row.map((cell, cIdx) => {
										const isHighlighted = highlightCell(rIdx, cIdx);
										...
										return (
											<td
												...
												style={{
													width: cellSize,
													height: cellSize,
													border: cell ? '1px solid #333' : '1px solid black',
													backgroundColor: cell ? 'white' : 'darkgray',
													textAlign: 'center',
													padding: 0,
													position: 'relative'
												}}
											>
												{number && (
													<div style={{ position: 'absolute', top: 1, left: 2, fontSize: 10, color: '#555' }}>{number}</div>
												)}
												{cell ? (
